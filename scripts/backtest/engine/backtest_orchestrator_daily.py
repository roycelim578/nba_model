...
def _award_core(ctx, day, budget, region_state, record, ref_budget=None):
    ...
    _carry_r = float(os.environ.get("CARRY_R", bo.CARRY_R))
    res = solve_award_v2(samples, cost_curves, portfolio_usd=budget, award_budget=_abk, kelly_fraction=1.0,
                         ...
                         guard_top_k=ctx.guard_top_k, carry_r=_carry_r, frac=frac)
    ...
    _conc_form = os.environ.get("CONC_FORM", bo.CONC_FORM)
    _cap_no = float(os.environ.get("CONC_S_CAP_NO", bo.CONC_S_CAP_NO))
    _cap_yes = float(os.environ.get("CONC_S_CAP_YES", bo.CONC_S_CAP_YES))
    _scale_params = ScaleParams(kelly_fraction=1.0, s_soft_no=bo.CONC_S_SOFT_NO,
                                s_soft_yes=bo.CONC_S_SOFT_YES, conc_form=_conc_form,
                                s_cap_no=_cap_no, s_cap_yes=_cap_yes)
